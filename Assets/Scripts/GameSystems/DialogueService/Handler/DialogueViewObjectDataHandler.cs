...
namespace GameSystems.DialogueDirectingService.Datas
{
    ...
    public class DialogueViewObjectDataHandler
    {
        private DialogueViewObjectGenerator DialogueViewObjectGenerator;

        private DialogueViewObjectData DialogueViewObjectData;

        public DialogueViewObjectDataHandler()
        {
            this.DialogueViewObjectData = new();
            this.DialogueViewObjectGenerator = new(this.DialogueViewObjectData, this.DialogueViewObjectData);
        }

        public bool TryGetAllPlugIn<T>(out T[] plugIns) where T : class
        {
            if (this.DialogueViewObjectData.TryGetAllPlugIn<T>(out plugIns)) return true;
            return false;
        }
        ...
        public bool TryGetPlugIn<T>(string key, out T plugIn) where T : class
        {
            ...
            if (this.DialogueViewObjectData.TryGetPlugIn<T>(key, out plugIn)) return true;
        ...
        {
            // 기존 데이터에 해당 값이 있는지 확인.
            if (this.DialogueViewObjectData.TryGetPlugIn<T>(viewObjectKey, out plugIn)) return true;
            // 없는 경우, 새로 만들어서 돌려주고자 함.
            else
            {
                // 만들기 실패. ( Prpefab이 없는 경우임 )
